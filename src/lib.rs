...
macro_rules! __pin_project_internal {
    ...
    (@make_proj_replace_block;
        ...
        // create drop guards for the newly created projected_type
        {
            ( $(
        ...
        $proj_vis fn $method_ident<'__pin>(
            ...
            unsafe {
                match self.$get_method() {
                    $(
                        $crate::__pin_project_internal!(@enum=>make_variant_match;
                            [Self]
                            [variant $variant
                             fields $($($(#[$pin])? $field)+)?]
                         ) => {
                            $crate::__pin_project_internal!(@enum=>make_variant_init;
                                [$proj_ty_ident]
                                [variant $variant
                                 fields $($($(#[$pin])? $field)+)?]
    ...
    (@enum=>make_variant_match;
        // could this be replaced with path
        [$ident:ident]
        [variant $variant:ident
        fields $( $($(#[$pin:ident])? $field:ident)+ )?]
    ) => {
        $ident::$variant $({
            $($field),+
        })?
    };
    (@enum=>make_variant_init;
        [$proj_ty_ident:ident]
        [variant $variant:ident
         fields $( $($(#[$pin:ident])? $field:ident)+ )?]
        [$projection:ident]
                    ...
                    $(
                        $crate::__pin_project_internal!(@enum=>make_variant_match;
                            ...
                            [variant $variant
                             ...
                             // this needs to be updated to pass [variant]
                            $crate::__pin_project_internal!{@make_proj_replace_block;
                                [$proj_ty_ident :: $variant]
                                $({
                                    $(
